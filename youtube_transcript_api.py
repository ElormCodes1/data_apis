...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('youtube_transcript.log', encoding='utf-8')
    ]
)
logger = logging.getLogger(__name__)

# Log startup
logger.info("🚀 YouTube Transcript API module loaded")
...
class YouTubeTranscriptRequest(BaseModel):
    url: str = Field(..., description="YouTube video URL", example="https://www.youtube.com/watch?v=3cEPgsbIdkw")
...
class YouTubeTranscriptError(BaseModel):
    success: bool
    error: str
    timestamp: str
...
async def get_youtube_transcript(request: YouTubeTranscriptRequest):
    ...
    This endpoint takes a YouTube video URL and returns the transcript text
    as a single string with newlines separating segments. The response is instant - no background processing required.
    """
    try:
        logger.info(f"Processing YouTube transcript request for URL: {request.url}")
        ...
        if 'v=' not in url:
            raise HTTPException(
                status_code=400, 
                detail="Invalid YouTube URL. Must contain video ID parameter 'v='"
            )
        
        video_id = url.split('v=')[1]
        
        # Get the page content to extract transcript parameters
        ...
        if not match:
            raise HTTPException(
                status_code=404,
                detail="Transcript not available for this video. The video may not have captions or transcripts enabled."
            )
        
        transcript_params = match.group(1)
        
        # Headers for the transcript API request
        headers = {
            'accept': '*/*',
        ...
        # JSON payload for the transcript API
        json_data = {
            'context': {
                'client': {
                    'hl': 'en',
                ...
                'adSignalsInfo': {
                    'params': [
                        ...
                        {'key': 'bih', 'value': '798'},
                        {'key': 'biw', 'value': '898'},
                        {'key': 'brdim', 'value': '0,158,0,158,1470,32,1470,798,898,798'},
                        {'key': 'vis', 'value': '1'},
                        {'key': 'wgl', 'value': 'true'},
        ...
        if transcript_response.status_code != 200:
            raise HTTPException(
                ...
                detail=f"Failed to fetch transcript. YouTube API returned status {transcript_response.status_code}"
            )
        ...
        transcript = []
        try:
            segments = transcript_data['actions'][0]['updateEngagementPanelAction']['content']['transcriptRenderer']['content']['transcriptSearchPanelRenderer']['body']['transcriptSegmentListRenderer']['initialSegments']
            
            for text in segments:
                try:
                    # Extract text from transcript segment
            ...
            raise HTTPException(
                status_code=500,
                detail="Failed to parse transcript data from YouTube response"
            )

        if not transcript:
            raise HTTPException(
                status_code=404,
                detail="No transcript segments found. The video may not have captions available."
            )

        logger.info(f"Successfully extracted {len(transcript)} transcript segments for video {video_id}")
        ...
        return YouTubeTranscriptResponse(
            ...
            message=f"Successfully extracted {len(transcript)} transcript segments"
        )
...
@router.get("/transcript", response_model=YouTubeTranscriptResponse)
async def get_youtube_transcript_get(url: str = Query(..., description="YouTube video URL")):
    """
    Extract transcript from a YouTube video URL (GET method).
    
    This endpoint takes a YouTube video URL as a query parameter and returns the transcript text
    as a single string with newlines separating segments. The response is instant - no background processing required.
    """
    request = YouTubeTranscriptRequest(url=url)
    return await get_youtube_transcript(request)

@router.get("/health")
async def health_check():
    """Health check endpoint for YouTube Transcript API"""
    return {
        "status": "healthy",

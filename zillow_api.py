...
async def get_task_results(
    ...
    """Get results and status from a Zillow scraping task"""
    ...
    # If task is not completed, return status information
    ...
    # Task is completed, return results
    ...
    download_urls = {
        "json": f"/zillow/download/{task_id}?format=json",
        "csv": f"/zillow/download/{task_id}?format=csv"
    }
...
async def download_results(
    task_id: str,
    format: str = Query("json", description="File format: 'json' or 'csv'")
):
    """Download Zillow scraping results in the specified format"""
    if task_id not in tasks:
        raise HTTPException(
            status_code=404,
            detail=f"Task {task_id} not found"
...
async def health_check():
    ...
    return {
        ...
        "endpoints": [
            "GET /zillow/for-sale - Search properties for sale (Async)",
            "GET /zillow/for-rent - Search rental properties (Async)",
            "GET /zillow/sold - Search sold properties (Async)",
            "GET /zillow/results/{task_id} - Get results and status",
            "GET /zillow/download/{task_id}?format=json|csv - Download results in specified format",
